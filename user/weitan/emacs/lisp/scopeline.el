...
(defvar scopeline--can-use-builtin-treesit (require 'treesit nil t)
  "If non-nil, we are can make use of elisp-treesitter instead of builtin.")
...
(defvar-local scopeline--overlays '()
  "List to keep overlays applies in buffer.")
(defvar scopeline-overlay-prefix "  ¤ "
  "Prefix to use for overlay.")
(defvar scopeline-min-lines 5
  "Minimum number of lines for block before we show scope info.")
(defface scopeline-face
  '((default :inherit font-lock-comment-face :foreground "#9f79ee"))
  ...
  (if (scopeline--use-builtin-treesitter)
      (let* ((root (treesit-buffer-root-node))
        ...
        (seq-map
         (lambda (x)
           (cons
            (treesit-node-start (cdr x)) (treesit-node-end (cdr x))))
         matches))
    ...
    (if (scopeline--use-builtin-treesitter)
        (intern
         (replace-regexp-in-string
          "-ts-mode$" "-mode" (symbol-name mode)))
      mode)
    scopeline-targets)))

(defun scopeline--show ()
  "Show all the scopeline items in buffer."
  (when-let* ((scopeline-targets-for-mode
               (scopeline--get-targets-for-mode major-mode))
              (query
               (progn
                 (string-join (seq-map
                               (lambda (ct)
              ...
              (matches
               (ignore-errors
                 (scopeline--get-query-matches query))))
    (seq-map
     (lambda (x) ; TODO: seq-map might not be the best option here
       (let* ((start-pos (car x))
              (end-pos (cdr x))
...
(define-minor-mode scopeline-mode
  ...
  (if scopeline-mode
      (if (scopeline--use-builtin-treesitter)
          (add-hook 'after-change-functions #'scopeline--redisplay
                    nil
                    t)
        (if scopeline--can-use-elisp-treesitter
            (progn
              ...
              (add-hook 'tree-sitter-after-change-functions
                        ...
                        t))
          (message
           "Unable to enable scopeline mode, tree-sitter not available.")))
    (progn
      (if (scopeline--use-builtin-treesitter)
          (remove-hook 'after-change-functions #'scopeline--redisplay
                       t)
        (remove-hook
         'tree-sitter-after-change-functions #'scopeline--redisplay
         t))
